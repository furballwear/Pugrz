...
import com.furballwear.apps.pugrz.api.FilesApi;
...
import com.furballwear.apps.pugrz.adapters.FilesAdapter;
...
public class RemoteFileDemo extends BaseActivity {
	...
	class GetFilesFromRemote extends AsyncTask<Void, FolderResponse, FolderResponse>{
		...
		protected FolderResponse doInBackground(Void... params) {
			...
			fileApi.addHeader("X-DreamFactory-Application-Name", IAppConstants.APP_PUGRZ);
	...
	class DlFileTask  extends AsyncTask<Object, FileResponse, FileResponse>{
			private String errorMsg;
			...
			protected FileResponse doInBackground(Object... params) {
				FilesApi fileApi = new FilesApi();
				fileApi.addHeader("X-DreamFactory-Application-Name", "pugrz");
				fileApi.addHeader("X-DreamFactory-Session-Token", session_id);
				fileApi.setBasePath(dsp_url);
				FileResponse fileToDownlod = (FileResponse) params[0];
				String containerName = "files/";
				String filePathOnServer = "uploaded_files/";
				String fileName = fileToDownlod.getName();
                String contenttype = fileToDownlod.getContent_type();
				try {
					fileName = URLEncoder.encode(fileName, "UTF-8"); // file name with space
				} catch (UnsupportedEncodingException e1) {
					e1.printStackTrace();
				...
				try {
					FileResponse resp = fileApi.getFile(containerName, filePathOnServer + fileName,true,true,true );
					log(resp.toString());
					return resp;
				} catch (ApiException e) {
					e.printStackTrace();
