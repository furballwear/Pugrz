 ...
 * {@link com.furballwear.apps.pugrz.ui.AttractionsActivity}.
 */
public class AttractionsGridPagerAdapter extends GridPagerAdapter
    ...
    private static final int PAGER_OPEN_ACTION_CHECKIN = 5;
    ...
    public Object instantiateItem(ViewGroup container, int row, final int column) {
        if (mAttractions != null && mAttractions.size() > 0) {
            ...
            switch (column) {
                ...
                case PAGER_OPEN_ACTION_CHECKIN:
                    // The "open on device" action
                    final ActionPage openActionPageCheckin = (ActionPage) mLayoutInflater.inflate(
                            R.layout.gridpager_action, container, false);

                    openActionPageCheckin.setOnClickListener(getStartActionClickListener(
                            attraction, Constants.START_ATTRACTION_PATH,
                            ConfirmationActivity.SUCCESS_ANIMATION));
                    openActionPageCheckin.setImageResource(R.drawable.ic_pugrz);
                    openActionPageCheckin.setText(mContext.getString(R.string.action_open_checkin));

                    container.addView(openActionPageCheckin);
                    return openActionPageCheckin;
            }
        }
